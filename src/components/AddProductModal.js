...
export default function AddProductModal({ handleSuccess, handleError }) {
  //suppliers
  ...
  const getSuppliers = async () => {
    try {
      const response = await fetch(
        "https://foodie-on-shelf-backend.vercel.app/Supplier/names"
      );
  ...
  const getStorage = async () => {
    try {
      const response = await fetch(
        "https://foodie-on-shelf-backend.vercel.app/storagelocation/storageAvail"
      );
      const jsonData = await response.json();
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://foodie-on-shelf-backend.vercel.app/product",
        {
          method: "POST",
    ...
    <div>
      ...
      >
        <Box className={"modalStyle"} sx={style}>
          <h1 style={{ marginBottom: 20 }}>Add a Product</h1>
          <TextField
            id="outlined-basic"
            label="Name"
            variant="outlined"
