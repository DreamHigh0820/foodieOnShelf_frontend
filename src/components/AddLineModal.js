...
export default function AddLineModal({ handleSuccess, handleError }) {
  //invoice dates
  ...
  const getDates = async () => {
    try {
      const response = await fetch(
        "https://foodie-on-shelf-backend.vercel.app/invoice/dates"
      );
  ...
  const getProducts = async () => {
    try {
      const response = await fetch(
        "https://foodie-on-shelf-backend.vercel.app/product/names"
      );
      const jsonData = await response.json();
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      console.log("to", inv_date);
      const response = await fetch("https://foodie-on-shelf-backend.vercel.app/line", {
        method: "POST",
    ...
    <div>
      ...
      >
        <Box className={"modalStyle"} sx={style}>
          <h1 style={{ marginBottom: 20 }}>Add a Line</h1>
          <TextField
            id="outlined-basic"
            label="Quantity"
            variant="outlined"
